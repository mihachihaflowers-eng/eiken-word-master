...
class Database:
    ...
    def insert_words(self, words: list):
        with self.connect() as conn:
            ...
            ids = [r[0] for r in conn.execute("SELECT id FROM words").fetchall()]
            conn.executemany(
                "INSERT OR IGNORE INTO progress (word_id) VALUES (?)",
                [(i,) for i in ids],
            )
        ...
        with self.connect() as conn:
            conn.execute(
                """UPDATE progress SET
                    ...
                    mastered_date  = NULL,
                    streak         = 0,
                    incorrect_count = MAX(incorrect_count, 1)
                WHERE word_id=?""",
                (word_id,),
            )
...
class QuizFrame(ctk.CTkFrame):
    ...
    def _answer(self, idx: int):
        ...
        self.app.db.update_session(self.session_id, self.current_q, self.answers)
    ...
    def _dont_know(self):
        ...
        meaning_str = "　/　".join(self.correct_meanings_all)
        self._feedback_lbl.configure(
            text=f"正解：{meaning_str}", text_color=C_WARN)
